...
public final class PluginProcessingContext implements PluginContext {
        ...
        if (securityCtx.isPresent()) {
            ...
            switch (entityId.getEntityType()) {
                ...
                case FARM:
                    ...
                    return;
                case LANDLOT:
                    validateLandlot(ctx, entityId, callback);
                    return;
                case RULE:
    ...
    private void validateLandlot(final PluginApiCallSecurityContext ctx, EntityId entityId, ValidationCallback callback) {
        if (ctx.isSystemAdmin()) {
            callback.onSuccess(this, ValidationResult.accessDenied(SYSTEM_ADMINISTRATOR_IS_NOT_ALLOWED_TO_PERFORM_THIS_OPERATION));
        } else {
            ListenableFuture<Landlot> landlotFuture = pluginCtx.landlotService.findLandlotByIdAsync(new LandlotId(entityId.getId()));
            Futures.addCallback(landlotFuture, getCallback(callback, landlot -> {
                if (landlot == null) {
                    return ValidationResult.entityNotFound("Landlot with requested id wasn't found!");
                } else {
                    if (!landlot.getTenantId().equals(ctx.getTenantId())) {
                        return ValidationResult.accessDenied("Landlot doesn't belong to the current Tenant!");
                    } else if (ctx.isCustomerUser() && !landlot.getCustomerId().equals(ctx.getCustomerId())) {
                        return ValidationResult.accessDenied("Landlot doesn't belong to the current Customer!");
                    } else {
                        return ValidationResult.ok();
                    }
                }
